...
void static SendByte(uint8_t byte) {
    // Copy the byte into a new volatile variable
    uint8_t sendValue = byte;
    // Get each individual bit from the byte starting from the most significant bit.
    int i = 0;
    for (i = 0; i < 8; i++) {
        uint32_t bit = (sendValue & 0b10000000) >> 7;
        // If the bit is 1, then send a 1 to the LCD, otherwise send a zero
        if (bit) {
            LCD_SEND_1;
        }
        else {
        ...
        // Bitshift the send value so that we move onto the next most signficant bit.
        sendValue = sendValue<<1;
        // Cycle the LCD clock so that it reads in and writes the data.
        LCD_SCLK_SET_HIGH;
        LCD_SCLK_SET_LOW;
    }
}
...
void static LCDCommandWrite(uint8_t command){
    // Enable LCD input
    LCD_ENABLE_MODULE;
    // Tell the LCD the type of data that is being transferred.
    SET_DC_FOR_COMMAND;
    // Send the data
    SendByte(command);
    // Disable LCD input
